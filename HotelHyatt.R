...
#Loading Aug 2014 Hyatt Dataset csv file
#HyattData <- read.csv('August.csv',header=TRUE)

#Saving this in RDS file format to mininmize the time for file upload
#saveRDS(HyattData,"HyattData.rds")

#Loading the dataset from RDS format
...
#Creating a subset with only US as the country
...
#Since California has highest number of detractors, creating subset with California state
Cali_subset <- subset(subsetUS, subsetUS$State_PL=='California')
summary(Cali_subset)
View(Cali_subset)
#setting the row numbers
rownames(Cali_subset) <- 1:nrow(Cali_subset)
...
#Checking if all the empty spaces have been replaced by NA
summary(Cali_subset)

#listing columns with NAs
colnames(Cali_subset)[colSums(is.na(Cali_subset)) > 0]
...
#To perform modeling, converting the following column data type to factors
...
############################################################################################
#Visulaization and Descriptive Analysis
############################################################################################
...
newdf2 <- newdf %>% select(NPS_Type,Brand_PL) %>% count(NPS_Type,Brand_PL) %>% spread(NPS_Type,n,fill = 0)
View(newdf2)

newdf2 <- data.frame(newdf2)

d.m <- melt(newdf2, id.vars= c("Brand_PL"))
View(d.m)

ggplot(d.m, aes(x=Brand_PL,y=value, fill = variable, color = variable,alpha = variable)) +   
  geom_bar(position = "identity", stat="identity")  + 
  scale_alpha_manual(values = c(0.7,0.3,0.1))

ggplot(d.m, aes(x=Brand_PL,y=value, fill = variable, color = variable)) +   
...
#getting latitude and longitude for all the points
...
##################################################################################################
#Visual - Cities in California and their NPS values
###################################################################################################
...
ggplot(ndf3, aes(x=Brand_PL,y=NPS, fill = POV_CODE_C, color = POV_CODE_C,alpha = POV_CODE_C)) +   
  geom_bar(position = "identity", stat="identity")  + scale_color_manual(values = c("lightblue","brown1")) +
  scale_fill_manual(values = c("lightblue","brown2")) + scale_alpha_manual(values = c(0.7,0.1)) 
  

ggplot(ndf3, aes(x=Brand_PL,y=NPS, fill = POV_CODE_C, color = POV_CODE_C)) +   
  geom_bar(position = "dodge", stat="identity")

##########################################################################################################
#Visual - Statewise NPS graph
##########################################################################################################
View(subsetUS)
d1 <- subsetUS[,c(43,78)]
...
#Use only for logit and linear modeling
...
################################
##Trying models with different combination of predictor variables
...
#Logit models didn't give good results, Linear regression models were deemed a better bit
#########################################################################################
...
View(actual)
View(ksvmpredNPS)
Kvar2 <- abs(as.numeric(actual) - as.numeric(ksvmpredNPS))
Kvar2
Kvar2 <- na.omit(Kvar2)
View(Kvar2)
...
####################################################################################################################
#RECCOMENDATIONS TO IMPROVE NPS. 
####################################################################################################################

#BASED ON THE ANALYSIS WE RECCOMEND THAT HYATT HOTEL CHAINS IN CALIFORNIA SHOULD FOCUS ON THE FOLLOWING:

#Improving the Hotel Condition, Guest Room Condition and Customer Service as they directly impact the Net Promoter Score.
#Improve the rooms ‘Guest Room King’ and ‘Guest Room Double,’ primarily in General Hyatt brand hotels.
#Center the Hotel Promotional Activities towards the age groups of 26-35, 36-45, and 46-55 as the detractors seem to be concentrated in that group.
#To improve the Hotel Condition, you can invest on improving facilities like Mini Bar, which will thereby have an impact on the NPS
